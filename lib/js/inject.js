    ...
    var settings = backgroundOptions.options.settings,
        ...
        guc = {
            ...
            getColorIcon: function(status, type){
                var mail = guc.isInbox ? "inbox" : "gmail";
                return settings.colors[mail][status][type];
            },
            getUnreadCount: function(){
                ...
                if (!guc.settings.useDOM){
                    ...
                    count = document.querySelectorAll(guc.selectors.inbox).length;
                } else {// its mail
                    label = document.querySelector(guc.selectors.mail) ? document.querySelector(guc.selectors.mail).getAttribute("aria-label") : "";
                    if (label){ // when trying to get the count from aria label e.g. a[aria-label^='Inbox'][aria-label$='unread']
                        count = guc.getCountRe.exec(label);
                        if (count && count.pop){
                            count = count[0];
                        }
                    } else {// when trying to get count from selector e.g. tr.apv td.apt span b
                        count = document.querySelectorAll(guc.selectors.mail).length
                    }
                }
                if ( (isNaN( parseInt(count) ) || count === 0 ) && guc.settings.forceHTTP ){
                    guc.log("GUC; can't find count from DOM, trying from HTTP", label);
                    guc.settings.currentUpdateType = "HTTP";
                    guc.getUnreadCountHTTP();
                } else {
                    guc.log("GUC; found count "+ count +" from DOM", label);
